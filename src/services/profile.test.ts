...
describe('partner companion profile', () => {
  it('uses the latest skin received for the current relationship', () => {
    const events = [skinEvent('oldRelationship01', 'mint'), skinEvent('currentRelation01', 'peach'), skinEvent('currentRelation01', 'sky')];
    expect(latestPartnerSkin(events, 'currentRelation01')).toBe('sky');
  });
