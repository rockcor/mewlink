...
Open `mewlink-pet-animation-master.aseprite` in Aseprite. It contains 844 frames and 78 named tags.

## Animation groups

- Eight-frame activity loops: `meeting`, `leisure`, `idle`, `rest`
- Work screens: `work-code`, `work-document`, `work-web`, `work-ai`; each stores exact `none`, `keyboard`, `pointer`, `both` input frames
...
`import-approved-work.lua` cuts and aligns the approved source sheet. `prepare-local-variants.lua` changes only the monitor pixels, expressions, cup, steam, and incoming arm for local variants. The AI screen uses a brand-neutral prompt-and-response graphic without provider logos or text. `motion-tween.py` creates motion-compensated in-betweens so faces and limbs travel instead of appearing twice. `build-animation-master.lua` imports those frames into the master project while preserving total timing, then exports the horizontal PNG strips under `public/pets/animations`.

Each transition starts with the exact final bitmap of its source loop and ends with the exact first bitmap of its destination loop. Work waits for both hands to return to their neutral frame and retains the current code, document, or web screen before leaving.
