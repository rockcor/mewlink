...
Open `mewlink-pet-animation-master.aseprite` in Aseprite. It contains 736 frames and 69 named tags.
...
`import-approved-work.lua` cuts and aligns the approved source sheet. `prepare-local-variants.lua` changes only the monitor pixels, expressions, cup, steam, and incoming arm for local variants. `motion-tween.py` creates motion-compensated in-betweens so faces and limbs travel instead of appearing twice. `build-animation-master.lua` imports those frames into the master project while preserving total timing, then exports the horizontal PNG strips under `public/pets/animations`.
