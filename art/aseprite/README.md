...
All frames use a 384×256 transparent canvas and the same bottom-center anchor. Work frames are imported from the approved two-arm source sheet. The viewer-left arm only touches the keyboard and the viewer-right arm only touches the mouse; the torso, head, ears, face, and devices stay fixed while input frames change.
